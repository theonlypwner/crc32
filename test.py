...
class ParserFunctions(unittest.TestCase):
  ...
  def test_flip(self):
    for k in test_data.flip:
      ...
      self.assertEqual(crc32.reverseBits(crc32.reverseBits(test_data.flip[k])), test_data.flip[k])
  ...
  def test_table(self):
    ...
    self.assertEqual(list(map(lambda x: x[0], crc32.table_reverse)), test_data.edb88320_reverse)
